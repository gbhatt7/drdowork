...
const Grid = ({ users, setUsers, setOnEdit }) => {
    ...
    const handleDelete = async (id) => {
        await axios
            .delete("http://localhost:8800/training/" + id)
            .then(({ data }) => {
                ...
                <thead>
                    <tr className="bg-gray-800">
                        <th className="px-12 py-2">
                            <span className="text-gray-200">training ID</span>
                        </th>
                ...
                <tbody className="bg-gray-200">
                    {users && users.map((item, i) => (
                        <tr className="bg-gray-50 text-center" key={i}>
                            ...
                            <td className="px-10 py-2"><span>{item.trainingname || "Unknown"}</span></td>
                            <td className="px-12 py-2"><span>{item.trainingplace || "Unknown"}</span></td>
                            <td className="px-12 py-2"><span>{item.trainingdate || "Unknown"}</span></td>
                            <td className="px-12 py-2">
                                <button className="cursor"><span className={`${item.trainingstatus === "Completed" ? 'bg-green-500' : item.trainingstatus === "Ongoing" ? 'bg-yellow-500' : 'bg-red-500'} text-white px-5 py-1 rounded-full`}>{item.trainingstatus || "Unknown"}</span></button>
                            </td>
